...
const REGISTRY: DestructiveEntry[] = [
  // ── Account, credentials, sessions ──────────────────────────────────────
  {
    ...
    unconfirmed: [
      {
        control: "research-mode switch, off position",
        reason:
          "Turning research mode off drops the stored acknowledgement receipt, which is regenerated by turning it back on and re-acknowledging. Nothing a person authored or measured is involved, so a dialog on a labelled switch would gate a preference rather than a loss.",
      },
    ],
  },
  {
