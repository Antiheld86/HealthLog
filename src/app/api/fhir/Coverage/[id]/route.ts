/**
 * GET /api/fhir/Coverage/{id} — FHIR R4 `read` of one Coverage from the
 * caller's own record.
 *
 * The id space is the export's own (`coverage-1`), stable for a given record and
...
export const GET = apiHandler(
  async (_request: NextRequest, ctx: { params: Promise<{ id: string }> }) => {
    ...
    annotate({ action: { name: "fhir.coverage.read" } });
    ...
    return readResponse([coverageResource(context.data, context.identity)].filter(
