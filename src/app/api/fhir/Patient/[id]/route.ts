/**
 * GET /api/fhir/Patient/{id} — FHIR R4 `read` of one Patient from the
 * caller's own record.
 *
 * The id space is the export's own (`patient-1`), stable for a given record and
...
export const GET = apiHandler(
  async (_request: NextRequest, ctx: { params: Promise<{ id: string }> }) => {
    ...
    annotate({ action: { name: "fhir.patient.read" } });
    ...
    return readResponse([patientResource(context.data, context.identity)], id);
  },
);
