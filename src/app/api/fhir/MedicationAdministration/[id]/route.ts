/**
 * GET /api/fhir/MedicationAdministration/{id} — FHIR R4 `read` of one MedicationAdministration from the
 * caller's own record.
 *
 * The id space is the export's own (`medadmin-7`), stable for a given record and
...
export const GET = apiHandler(
  async (_request: NextRequest, ctx: { params: Promise<{ id: string }> }) => {
    ...
    annotate({ action: { name: "fhir.medicationadministration.read" } });
    ...
    return readResponse(
      medicationAdministrationsFromReportData(context.data, {
