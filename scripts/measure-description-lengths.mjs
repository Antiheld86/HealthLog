...
const locale = process.argv[2]?.startsWith("--") ? "de" : (process.argv[2] ?? "de");
...
console.log(`>= 2 sentences: ${multi.length}, >= 3: ${entries.filter(([, v]) => sentences(v) >= 3).length}`);
console.log(`\nLONGEST ${top}:`);
for (const [key, value] of [...entries]
  .sort((a, b) => b[1].length - a[1].length)
  .slice(0, top)) {
  console.log(`${String(value.length).padStart(4)}ch ${sentences(value)}S ${key}`);
